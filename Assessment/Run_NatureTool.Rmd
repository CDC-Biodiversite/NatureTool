...
exchange_rates <- read_xlsx("../data/2024_INSEE_Exchange_rates_EUR.xlsx", sheet = "Data_for_NatureTool") # Update file if exhange rates have changed
...
# Computation
portfolio_impacts <- get_impacts(portfolio_complete_data, all_impacts)
...
dependencies_average <- portfolio_dependencies_average$portfolio_dependencies
...
# Average dependencies by entity (Name business & Group) for each ISIN in the input file and its subsidiaries
dependencies_critical <- portfolio_dependencies_critical$portfolio_dependencies
# Aggregated dependencies for each ISIN of the input file (no subsidiaries breakdown)
...
writeData(wb, sheet = "portfolio_impacts", portfolio_impacts_by_isin)
